            ...
            <div className={classes.blogs}>
              {filteredBlogs?.map((blog) => (
                <div key={blog._id} className={classes.blog}>
                  <Link to={`/blogDetails/${blog?._id}`} className={classes.title}>
                    <h4 className={classes.title}>{blog?.title}</h4>
                  </Link>
                  <Link to={`/blogDetails/${blog?._id}`}>
                    <img src={`http://localhost:5000/images/${blog?.photo}`} alt="" className={classes.imgg}/>
                  </Link>
                  <div className={classes.blogData}>
                    ...
                    <div className={classes.authorAndCreatedAt}>
                      ...
                      <span><span>Created:</span> {new Date(blog?.createdAt).toDateString()}</span>
                    </div>
